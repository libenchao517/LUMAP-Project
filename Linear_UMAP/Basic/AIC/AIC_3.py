################################################################################
# 基本实验1-3：2DUMAP最优降维维度测算实验
#
# 实验目的：测试以下数据集在Two Dimensional UMAP上的效果
# 评价指标：AIC
################################################################################
# 添加路径和消除警告
import sys
...
# 导入关键变量
config = LUMAP_config()
AI = Akaike_Information_Criterion(filename='AIC-3-2DUMAP', func_name='2DLUMAP')
...
for d, ds in enumerate(config.basic_data):
    model = factory(
        func_name="2DUMAP", data_name=ds, train_size=0.1,
        random_state=config.split_state, return_time=True,
        sec_part="AIC", sec_num=3)
    ...
    for i in range(1, config.cut_dimension+1):
        model.Product_Linear_UMAP_Object(
            ...
            is_Supervised=False, init=config.LUMAP_2D_init,
            transform_mode=config.transform_mode)
        model.Linear_2DUMAP_Object.fit_transform(
            X_train=model.X_train, X_test=model.X_test,
            T_train=model.T_train, T_test=model.T_test, label=None,
