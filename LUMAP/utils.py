...
################################################################################
class LUMAP_Supervised_Labels:
    """
    用于生成Supervised Linear UMAP和Supervised 2DUMAP的新标签
    """
    ...
    def Make_label(self, data, target, label_n, target_n):
        ...
        :param data: 数据集
        :param target: 标签
        :param label_n: 标签向量的维度
        :param target_n: 类别的数量
        :return:
        """
    ...
    def choice_model(self, data, target, label_n=10):
        ...
        if self.model.upper() == 'PCA':
            ...
            if data.shape[0] < data.shape[1]:
                data = PCA(n_components=data.shape[0]-1).fit_transform(data)
