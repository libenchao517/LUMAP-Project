...
from LUMAP.layouts import optimize_layout_linear_euclidean
...
def simplicial_set_embedding(
        ...
        output_metric_kwds={},
        euclidean_output=True, parallel=False,
        verbose=False,
        tqdm_kwds=None
):
    graph = graph.tocoo()
    ...
    if init.lower() == "random":
        map_matrix = np.random.random((data.shape[1], n_components)).astype(np.float32)
    elif init.lower() == "pca":
        pca_init = PCA(n_components=n_components)
        pca_init.fit(data)
        map_matrix = pca_init.components_.T
    ...
    if euclidean_output:
        map_matrix = optimize_layout_linear_euclidean(
            map_matrix,
            data,
            head,
            tail,
...
################################################################################
class LUMAP(BaseEstimator):
    """
    Linear Uniform Manifold Approximation and Projection
    Supervised Linear Uniform Manifold Approximation and Projection
    """
    def __init__(
        self,
        n_neighbors=15,
    ...
    def fit(self, X, y=None, force_all_finite=True):
        ...
        if self.transform_mode in MODE_LIST:
            ...
            self.map_matrix_, aux_data = self._fit_embed_data(self._raw_data[index], epochs, init, random_state)
        if self.verbose:
