...
for idx in index:
    for rst in Results_list:
        ...
        rst_idx = pd.read_excel(result_path, "apply", header=0, index_col=0)
...
result_total = np.zeros((num_rst, num_method, num_data))
for i, rst in enumerate(Results_list):
    result_path = os.path.join(rst, "Result-Files", "time" + ".xlsx")
...
result_std = np.nanstd(result_total, axis=0)
result_std = pd.DataFrame(result_std, index=rst_idx.index, columns=rst_idx.columns)
with pd.ExcelWriter(project+ "-time.xlsx") as writer:
    result_mean.to_excel(writer, sheet_name="Mean")
    result_std.to_excel(writer, sheet_name="Std")
MT.Make(project+ "-time.xlsx")
shutil.move(fromdir + "/" + project + "-time.xlsx", todir)

test_index = ["KNN", "SVM", "FMS", "SHS", "NMI"]

for ti in test_index:
    result_total = np.zeros((num_rst, num_method, num_data))
    for i, rst in enumerate(Results_list):
        result_path = os.path.join(rst, "Result-Files", ti + ".xlsx")
        rst_idx = pd.read_excel(result_path, "test", header=0, index_col=0)
        result_idx = rst_idx.to_numpy()
        result_total[i, :, :] = result_idx
    result_mean = np.nanmean(result_total, axis=0)
    result_mean = pd.DataFrame(result_mean, index=rst_idx.index, columns=rst_idx.columns)
    result_std = np.nanstd(result_total, axis=0)
    result_std = pd.DataFrame(result_std, index=rst_idx.index, columns=rst_idx.columns)
    with pd.ExcelWriter(project + "-" + ti + "-oos.xlsx") as writer:
        result_mean.to_excel(writer, sheet_name="Mean")
        result_std.to_excel(writer, sheet_name="Std")
    MT.Make(project + "-" + ti + "-oos.xlsx")
    shutil.move(fromdir + "/" + project + "-" + ti + "-oos.xlsx", todir)
